            ...
            <div class="d-flex flex-column col-lg-9 col-md-12 col-sm-12">
                ...
                @foreach($services as $service)
                        ...
                        <div class="media media-new d-flex flex-column" style="margin-bottom: 10px">
                            ...
                            <a class="new_img " href="{{ route('service_content',$service->ServiceID)}}">
                                <img class="media-object w-100 h-100" src="{{ asset($service->ServiceImage) }}" width="150">
                            </a>
                            </div>
                            <div class="new-body pl-1 pt-3 w-100">
                            <a class="new_title text-center" href="{{ route('service_content',$service->ServiceID)}}"><h5>{{$service->ServiceTitle}}</h5></a>
                            <span class="new_abstract text-dark ">{{$service->Abstract}}</span>
                            <p class="new_date text-dark text-right">{{$service->create_at}}</p>
                            </div>
                        </div>                    
                        </li>
                    @endforeach
